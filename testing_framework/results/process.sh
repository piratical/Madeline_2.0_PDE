...
do
	...
	then
		...
		echo "<p style='font-family:Ubuntu;font-size:20px;font-style:italic;color:#0c0;'>PASS: ${BASH_REMATCH[1]}</p>" >> summary.report
		((passing++))
...
total=$((passing+differing))
statistics="${passing} passed ($((100*passing/total))%) ${differing} differ ($((100*differing/total))%)"
date "+<p style='font-family:Ubuntu;font-size:20px;font-style:italic'>Test results in this document were last run on %Y.%m.%d at %H:%Mh" > report.date
echo " :: ${statistics}.</p>">> report.date
echo " $statistics"
echo ""
